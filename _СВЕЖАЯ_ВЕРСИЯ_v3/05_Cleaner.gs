/**
 * 05_Cleaner.gs
 * Нормализация и фильтрация ключевых слов.
 */
...
function cleanAllKeywords_(wbReport, log) {
  ...
  Object.keys(wbReport.keywordsBySku).forEach(sku => {
    ...
    block.rows.forEach(row => {
      ...
      cleaned.push({
        keyword: String(kwRaw || '').trim(), normalized: norm,
        freq, freqPrev: freqPrv, cart, cartPrev: cartPrv,
        order, orderPrev: orderPrv, isOurs: (String(sku) === ourSku)
      });
      totalOut++;
    });
    const dedup = {};
    cleaned.forEach(it => {
      if (!dedup[it.normalized] || dedup[it.normalized].freq < it.freq) dedup[it.normalized] = it;
    });
    result[sku] = Object.values(dedup);
  });
  log.step('🧹 Нормализация: ' + totalIn + ' → ' + totalOut);
  return result;
}
