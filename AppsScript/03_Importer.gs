 ...
 *
 * v2: универсальный поиск листов конкурентов через содержание имени +
 *     извлечение артикула из A1, не по префиксу "N ".
 * ============================================================================
 */
...
function readWbReport_(fileId, log) {
  ...
  // Утилита: найти лист, чьё имя начинается с указанного префикса
  ...
  // НОВАЯ ЛОГИКА: ищем ВСЕ листы, чьё имя содержит "Поисковые запросы по артикул"
  // (включая префиксы "2 ", "3 ", и без префикса). Исключаем агрегат "по всем артикул".
  // Артикул извлекаем из ячейки A1 каждого листа.
  const keywordsBySku = {};
  const candidates = sheets.filter(s => {
    const name = s.getName();
    if (!/Поисковые запросы по артикул/i.test(name)) return false;
  ...
  candidates.forEach(s => {
    ...
    if (lr < 2) return;
    // Артикул из строки 1 (там полный заголовок отчёта)
