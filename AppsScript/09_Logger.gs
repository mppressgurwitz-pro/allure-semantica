 ...
 * 09_Logger.gs
 * ============================================================================
 * Журнал запусков. Каждое исполнение пайплайна пишет одну строку в _Logs:
 *   timestamp, file, status, duration_ms, шаги (json).
 * Хранение ограничено LIMITS.MAX_LOG_ROWS — старые строки обрезаются.
 * ============================================================================
 */
...
function newLogContext_(label) {
  ...
  ctx.step = function(m) {
    ...
    ctx.steps.push('[' + ts + '] ' + m);
    SpreadsheetApp.getActive().toast(m, 'Импорт', 3);
    Logger.log(m);
  };
  ctx.error = function(m) {
    ctx.errors.push(m);
    Logger.log('ERROR: ' + m);
  };
