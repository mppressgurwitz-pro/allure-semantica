 ...
 *
 * SemanticCore: уникальный набор ключевых слов с агрегированной частотностью
 *               по всем артикулам, с метками "наш" / "конкурент".
 * MissingKeywords: запросы, которые приносят трафик/заказы конкурентам,
 *               но отсутствуют (или с низкой частотой) у нас.
 * ============================================================================
 */

/**
 * @param {Object} cleanedKeywords sku → [{normalized, freq, ...}]
 * @return {Array<Object>} semCore [{normalized, totalFreq, ourFreq, competitorsFreq, ourCart, ourOrder, sources:[sku]}]
 */
...
/**
 * Missing keywords: запросы, по которым у нас нет трафика, а у конкурентов есть.
 * Сортировка по competitorsFreq DESC.
 */
...
function writeMissingKeywords_(missing, log) {
  ...
  if (missing.length) {
    // Приоритет = квартиль по freq × (1 + bestOrder/100)
...
function recomputeMissingOnly() {
  ...
  try {
    // Грузим из помойки _KeywordsRaw
