...
function pasteZaprosy_(ss, wbReport, log) {
  ...
  if (out.length) {
    writeChunked_(sh, 2, 1, out, log, 'Запросы');
  }
...
/**
 * Чанкованный write с ретраями на случай таймаутов Sheets API.
 */
function writeChunked_(sh, startRow, startCol, rows2d, log, label) {
  if (!rows2d || !rows2d.length) return;
  const CHUNK = CONFIG.LIMITS.SHEET_WRITE_CHUNK || 300;
  ...
  for (let i = 0; i < rows2d.length; i += CHUNK) {
    ...
    while (true) {
      try {
        sh.getRange(startRow + i, startCol, slice.length, cols).setValues(slice);
        break;
      } catch (e) {
        attempt++;
        if (attempt > 3) throw e;
        log.step('⏳ ' + (label||'write') + ' chunk #' + (i/CHUNK+1) + ' retry ' + attempt + ': ' + e.message);
        Utilities.sleep(800 * attempt);
      }
    }
    if ((i/CHUNK) % 5 === 4) SpreadsheetApp.flush();
  }
}
