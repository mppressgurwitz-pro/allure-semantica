...
function rebuildDashboard_(wbReport, semCore, missing, scored, log) {
  ...
  header('🪄 Рекомендации Claude (после кнопки «Сделать семантику + SEO»)', '#9900ff');
  const adviceLinks = [
    ['Черновик SEO-текста',       CONFIG.ADVICE_TABS.SEO_DRAFT],
    ['Стратегия рекламы',         CONFIG.ADVICE_TABS.AD_STRATEGY],
    ['Логистика и склады',        CONFIG.ADVICE_TABS.LOGISTICS],
    ['Стратегия рекламной полки', CONFIG.ADVICE_TABS.SHELF_STRATEGY],
    ['Разбор конкурентов',        CONFIG.ADVICE_TABS.COMPETITOR_ANALYSIS],
    ['Трекер внедрения',          CONFIG.ADVICE_TABS.IMPLEMENTATION]
  ];
  adviceLinks.forEach(l => {
    sh.getRange(row, 1).setValue(l[0]).setFontWeight('bold');
    sh.getRange(row, 2).setFormula('=HYPERLINK("#gid=' + getSheetGid_(ss, l[1]) + '"; "Открыть ' + l[1] + '")');
    row++;
  });
